...
inline Vector2 vector2Normalize(const Vector2 _v)
{
	Vector2 result = (Vector2){0, 0};
	const f32_t length = sqrtf(POW2(_v.x) + POW2(_v.y));

	if (length > 0)
	{
		const f32_t ilength = 1.0f / length;
		result.x = _v.x * ilength;
		result.y = _v.y * ilength;
	}
...
OApp::OApp()
{
	...
	this->cursor_state = OAppCursorState::cursor_up;

	OAppSettings__loadFromFile(&this->settings, OAPP_SETTINGS_FILE_PATH);
	if (this->settings.font_path == "default")
	{
		this->settings.font_path = OAPP_FONT_DEFAULT_PATH;
	};
...
i32_t OApp::run()
{
	...
	while (!this->window_quit)
	{
		// Update.
		{
			...
			{
				const Vector2 cursor_position_window = GetMousePosition();
				Vector2 cursor_position_speed =
					(Vector2){
						(cursor_position_window.x * this->settings.cursor_speed),
						(cursor_position_window.y * this->settings.cursor_speed)};


				this->cursor_position = vector2Follow(
					this->cursor_position,
					cursor_position_speed,
			...
			{
				this->cursor_state =
					((this->cursor_state != cursor_up) ? cursor_down : cursor_pressed);
			}
			else
			{
				this->cursor_state = cursor_up;
			};
...
void OApp::layerIntro()
{
	...
	if (this->cursor_state != OAppCursorState::cursor_up)
	{
...
void OApp::layerSettings()
{
	...
	// Cursor speed.
	{
		...
		basic_slider(TextFormat("%s: %.0f%s", OAL__get(&this->language, "slider_cursor_speed"), this->temp_settings.cursor_speed * 100.0f, "%"),
					 offset.x + button_position.x,
					 offset.y + button_position.y,
					 button_width,
					 this->style.font_size_text,
		...
		this->temp_settings.cursor_speed = new_cursor_speed;
	};
	...
	// Use and Close buttons.
	{
		...
		{
			if (this->temp_settings.cursor_speed !=
				this->settings.cursor_speed)
			{
				const float cursor_scale = this->settings.cursor_speed / this->temp_settings.cursor_speed;
				d_clogf_fl("%f", cursor_scale);
				this->cursor_position.x *= cursor_scale;
				this->cursor_position.y *= cursor_scale;
			};
...
void OApp::layerGame()
{
	...
	for (usize_t i = 0; i < this->circles.size(); i++)
	{
		...
		if (is_cursor_circle_intersect &&
			this->cursor_state == OAppCursorState::cursor_pressed)
		{
			if (this->game_stats.status == OAppGameStatus::gamestatus_play)
			{
	...
	if (this->game_stats.status == OAppGameStatus::gamestatus_begin)
	{
		...
		if (this->cursor_state == OAppCursorState::cursor_pressed)
		{
			this->game_stats.status = OAppGameStatus::gamestatus_play;
		};
